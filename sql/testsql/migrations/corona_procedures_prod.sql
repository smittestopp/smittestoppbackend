...
as
set nocount on
declare @year int, @month tinyint, @day tinyint, @hour tinyint, @second tinyint, @partition tinyint, @copy tinyint, @daypart tinyint;
...
select @second = 0, @partition = @partitionid, @copy = 0;

-- Set the loop breaks to one hour ago
select @maxyear = datepart(year,dateadd(hh,-1,getdate())),
	@maxmonth = datepart(month,dateadd(hh,-1,getdate())),
	@maxday = datepart(day,dateadd(hh,-1,getdate())),
	...
	--	while @partition < 4 begin
			while @copy < 20 begin
				set @path = ''+
						...
						case when len(cast(@hour as varchar(2)))=1 then '0'+cast(@hour as varchar(2)) else cast(@hour as varchar(2)) end
						+'-'+
						case when len(cast(@second as varchar(2)))=1 then '0'+cast(@second as varchar(2)) else cast(@second as varchar(2)) end
						+'.'+
...
{'',''}},{'') +'']'') WITH (uuid   varchar(36)   ''$.SystemProperties.connectionDeviceId'',
		...
		--end
		--select @second = @second + 1, @partition = 0
		select @second = @second + 1, @copy = 0
	end
	select @hour = @hour + 1, @second=0
end
