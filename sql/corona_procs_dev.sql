...
*/
/*
Get UUID list who has had no activity since (datetime)
-- return lastactivtytime, uuid

declare @earliestdate datetime2(0) = dateadd(day,-7, getdate())
exec getLastActivityBefore @earliestdate
*/
drop procedure getLastActivityBefore
go
...
;
go
grant execute on getLastActivityBefore to [FHI-Smittestopp-Sletteservice-Dev];


-- Delete all data for a UUID
drop procedure deleteforUUID 
go
/*
...
create procedure deleteforUUID (
	@uuid varchar(36))
as
	--delete from dluserdatastaging where uuid = @uuid;
	--delete a from gpsevents a join uuid_id u on a.id = u.id and u.uuid = @uuid;
	--delete a from btevents a join uuid_id u on a.id = u.id and u.uuid = @uuid; 
	delete a from uuid_activity a join uuid_id u on a.id = u.id and u.uuid = @uuid; 
	delete from uuid_id where uuid = @uuid;
go

grant execute on deletedatafordeleteduuids to [FHI-Smittestopp-Sqlimport-Sletteservice]
/*
Delete data for deleted uuids'

exec deletedatafordeleteduuids
*/
drop proc deletedatafordeleteduuids
go
...
grant execute on deletedatafordeleteduuids to [FHI-Smittestopp-Sqlimport-Prod]

-- Delete all data more than 30 days old
drop procedure delete30days
go
...
delete 
select count(*) from uuid_id where uuid not in (select uuid from dluserdatastaging with(nolock))
...
go
/*
select * into dluserdatastaging_old from dluserdatastaging where daypart <= datepart(dayofyear,(dateadd(dayofyear,-31,getdate())));
select * into btevents_old from btevents where daypart <= datepart(dayofyear,(dateadd(dayofyear,-31,getdate())));
select * into gpsevents_old from gpsevents where daypart <= datepart(dayofyear,(dateadd(dayofyear,-31,getdate())));

select count(*) from dluserdatastaging_old where uuid not in (select uuid from dluserdatastaging)
select min(daypart) from dluserdatastaging
select min(daypart) from btevents
select min(daypart) from gpsevents
select count(*) from btevents_old
select count(*) from gpsevents_old

select datepart(dayofyear,(dateadd(dayofyear,-31,getdate())))
select datepart(dayofyear,getdate())
*/

 
grant execute on delete30days to [FHI-Smittestopp-Sqlimport-Prod]
-- App logging

/*
REMEMBER TO RE-RUN THE PERMISSIONS GRANT STATEMETNS WHEN MAKING CHANGES
*/
drop procedure apploginsert
go
...
;
GO
grant execute on apploginsert to [FHI-Smittestopp-ServiceAPI-Dev]
go
/*
REMEMBER TO RE-RUN THE PERMISSIONS GRANT STATEMETNS WHEN MAKING CHANGES
*/
drop procedure applogfetch
go
...
END
GO
grant execute on applogfetch to [FHI-Smittestopp-ServiceAPI-Dev]
go

/*
-- GPS egress / GDPR show-me-my-data

-- Example: 

select top 1000 * from dluserdata

exec getdatabyUUIDList '4e3b995e80a84c4faa01fe731af0fb11',N'2020-04-16T07:00:00Z',N'2020-05-17T07:59:59.999999Z',1,30

exec getdatabyuuidlist '3db98d5e59da4181bb901e45b9dadca4,69d2945eeb1c4692a0e292979dc94de7,8963985e331f40aaacf87a71710a3c55',N'2020-04-17T00:00:00Z',N'2020-04-17T08:59:59.999999Z',1,30

*/
drop procedure getdatabyUUIDList
go
/*
REMEMBER TO RE-RUN THE PERMISSIONS GRANT STATEMETNS WHEN MAKING CHANGES
*/
...
--grant execute on getdatabyUUIDList to [FHI-Smittestopp-ServiceAPI-Dev];
--grant execute on getdatabyUUIDList to coronaanalyst
...
/*
latestActivityForUUID
exec latestActivityForUUID '36ab995e8cf048c79ab0b122e25739c8'
*/
drop proc latestActivityForUUID
go
...
;
go
--grant execute on latestActivityForUUID to [FHI-Smittestopp-ServiceAPI-Dev];
grant execute on latestActivityForUUID to [FHI-Smittestopp-ServiceAPI-Prod];
grant execute on latestActivityForUUID to [FHI-Smittestopp-Sletteservice-Prod];
--grant execute on latestActivityForUUID to [FHI-Smittestopp-Sletteservice-Dev];
...
grant execute on updatemonitors to [FHI-Smittestopp-Sqlimport-Prod]
go
/*
ADLSImporter - import datalake files
*/
drop proc adlsimporter
go
...
select @maxyear = datepart(year,dateadd(hh,-2,getdate())),
	...
	@maxhour = datepart(hour,dateadd(hh,-1,getdate()))
...
grant execute on adlsimporter to [FHI-Smittestopp-Sqlimport-Dev]
go
...
grant execute on btimporter to [FHI-Smittestopp-Sqlimport-Dev]
go
...
declare @maxyear int;
select @maxyear = datepart(year,dateadd(hh,-1,getdate()))
select theyear = datepart(year,dateadd(dd,daypart-1,cast(cast(@maxyear as char(4)) as datetime))), 
themonth = datepart(month,dateadd(dd,daypart-1,cast(cast(@maxyear as char(4)) as datetime))),
theday = datepart(day,dateadd(dd,daypart-1,cast(cast(@maxyear as char(4)) as datetime))),
thehour = max(hourpart) from dluserdatastaging with(nolock) -- max hour plus one gives us the new hour
where daypart in (select max(daypart) from dluserdatastaging with(nolock))
group by daypart;
...
grant execute on gpsimporter to [FHI-Smittestopp-Sqlimport-Dev]
go
...
Duplicates may arrive when a device is resending data, not knowing that data previously was sent. As there is
a awful cost for checking if data already exists at insert time, we accept a certain amount of dupes and the functions still work. 
But we want to minimize the impact of redundant data (slower queries), so a little cleanup is best to do now and then.

I wrote it like this so it's possible to execute one step at a time manually without too much locking and investigate / understand 
what duplicate we have and benefit in troubleshooting.

exec deDuplicator
*/

drop procedure deDuplicator
go
...
drop procedure deDuplicatorGPS
go
...
while @hr < 24 begin
	...
	print 'Hour=' + cast(@hour as varchar(20)) + ', count=' + cast(@count as varchar(20))
...
drop procedure deDuplicatorBT
go

create procedure deDuplicatorBT (@daypart int)
as
declare @hr int = 0;
declare @count int;
...
while @hr < 24 begin
	...
	SELECT id, pairedtime, rssi, txpower, daypart, pairedid
	into btevents_dupes
	FROM btevents
	where daypart = @daypart
	and datepart(hh,pairedtime) = @hr
	GROUP BY id, pairedtime, rssi, txpower, daypart, pairedid
	HAVING count(*) > 1
	
	DELETE t
	FROM btevents t join btevents_dupes t2
	on t.id = t2.id 
		and t.pairedtime = t2.pairedtime
		and t.rssi = t2.rssi
		and t.txpower = t2.txpower
		and t.daypart = t2.daypart
		and t.pairedid = t2.pairedid;
	
	insert into btevents(id, pairedtime, rssi, txpower, daypart, pairedid)
	select id, pairedtime, rssi, txpower, daypart, pairedid from btevents_dupes;
	
	drop table btevents_dupes;
...
/*
Populate agg_gpsevents table

exec gpsevents_aggregator 108

declare @i int = 109
while @i < 138 begin
	exec gpsevents_aggregator @i
	set @i = @i + 1
end

*/
drop procedure gpsevents_aggregator
go
...
/*
Reduce event frequency when we have multiple events within same 10sec interval
Will have a few hundred remaining rows that have duplicate timeto in dev

exec deFrequencyFier
*/
drop procedure deFrequencyFierGPS
go

create procedure deFrequencyFierGPS (@daypart int)
as
...
exec deFrequencyFierBT 117
-- 114 har veldig mye highfreq, skipper den
*/
drop procedure deFrequencyFierBT
go
...
/* 
Fix for same timefrom. Some phones may have this error.

exec deDuplicateTimefrom
*/
drop proc deDuplicateTimefrom
go
