...
*/
create table applog (
	timest datetime primary key clustered,
	phoneNumber nvarchar(20),
	personorrole nvarchar(200),
	organization nvarchar(200),
	legalmeans nvarchar(200)
)
go


/*
cleanup data quality examples
*/
select * from userevents_bluetooth where 
update userevents_bluetooth set paireddeviceid = dbo.RemoveNonASCII(paireddeviceid)

/*
Cross-database queries example
*/

CREATE database scoped CREDENTIAL smittecred WITH IDENTITY = 'smittedbo',
    SECRET = '<insert user password her>' ;
GO

CREATE EXTERNAL DATA SOURCE remotesmittedb
WITH
(
 TYPE=RDBMS,
 LOCATION='sqlserver-smittestopp-dev.database.windows.net',
 DATABASE_NAME='sql01-smittestopp-dev_temp',
 CREDENTIAL= smittecred
);

CREATE EXTERNAL TABLE [dbo].[userevents_bluetooth](
	[uuid] varchar(36) NOT NULL,
	[paireddeviceid] varchar(36) NOT NULL,
	[pairedtime] datetime2(0) NOT NULL,
	[rssi] int NOT NULL,
	[txpower] int NOT NULL,
	daypart smallint NOT NULL
) WITH (DATA_SOURCE = remotesmittedb)
GO
