...
BEGIN
	...
	BEGIN
		...
		IF (UNICODE(@nchar) between 48 and 57 -- numbers
			...
			or UNICODE(@nchar) between 45 and 46) -- bindestrek
...
return (
with filter as (
	SELECT distinct id , timefrom , timeto , latitude , longitude , accuracy FROM gpsevents with(nolock)
	WHERE daypart = datepart(dayofyear, @dateday) 
...
SELECT top 100 percent
	...
	b.rssi		 AS bt_rssi,
	b.txpower	 AS bt_txpower
FROM filter a JOIN btevents b with(nolock)
ON a.id = b.id
   AND b.daypart = datepart(dayofyear, @dateday)
   AND b.pairedtime BETWEEN dateadd(ss, @timeslack_before , a.timefrom) AND dateadd(ss , @timeslack_after , a.timeto) 
join uuid_id u1 with(nolock) on a.id = u1.id
join uuid_id u2 with(nolock) on b.pairedid = u2.id
ORDER BY u1.uuid, u2.uuid, a.timefrom, b.pairedtime
)
...
return (
with filter as (
	select distinct u.id,timefrom,timeto,latitude,longitude,accuracy from gpsevents a
	join uuid_id u on a.id = u.id
	where u.uuid = @uuid
	AND timefrom >= @timefrom
	AND timeto   <= @timeto
...
create function getIntersectedTrajectoriesSpeed3(
	@uuid varchar(40), 
	@timefrom datetime2(0), 
	@timeto datetime2(0),
	@distance float = 15, -- what distance to other UUID's we are searching for
...
return (
with filter as (
	select id,timefrom,timeto,latitude,longitude,accuracy,speed,distancemeters,[m/s] from getTrajectorySpeed3(@uuid,@timefrom,@timeto)
	)
SELECT top 100 percent c1.uuid as MyUUID, 
	a.latitude as MyLocationLat,
	a.longitude as MyLocationLong,
	a.accuracy as MyAccuracy,
	a.speed,
	...
	round(dbo.fnGetDistance(a.latitude,a.longitude,b.latitude,b.longitude),2) as dist
FROM filter a 
JOIN uuid_id c1 on a.id = c1.id
JOIN agg_gpsevents b 
	ON b.latitude BETWEEN a.latitude - (@distance*0.001 / 111.045) 
		AND a.latitude + (@distance*0.001 / 111.045)
		and b.longitude BETWEEN a.longitude - (@distance*0.001 / (111.045 * COS(RADIANS(a.latitude)))) 
		AND a.longitude + (@distance*0.001 / (111.045 * COS(RADIANS(a.latitude))))  
...
return (
with filter as (
	select distinct uuid,u.id,timefrom,timeto,latitude,longitude,accuracy from gpsevents with(nolock)
	join uuid_id u on gpsevents.id = u.id
	where u.uuid = @uuid
	AND timefrom >= @timefrom
	AND timeto   <= @timeto
	AND daypart between datepart(dayofyear,@timefrom) and datepart(dayofyear,@timeto)
	)
SELECT top 100 percent u.uuid, b.timefrom, b.timeto, b.latitude, b.longitude, b.accuracy, b.speed 
FROM filter a JOIN gpsevents b with(nolock)
     ON  b.latitude  BETWEEN a.latitude -  (@distance*0.001 / 111.045) 
     		         AND a.latitude +  (@distance*0.001 / 111.045)
     AND b.longitude BETWEEN a.longitude - (@distance*0.001 / (111.045 * COS(RADIANS(a.latitude)))) 
		         AND a.longitude + (@distance*0.001 / (111.045 * COS(RADIANS(a.latitude))))  
     ...
     AND a.id <> b.id
     AND b.daypart between datepart(dayofyear,@timefrom) and datepart(dayofyear,@timeto)
join uuid_id u with(nolock) on b.id = u.id
ORDER BY u.uuid, b.timefrom asc
);
...
FROM gpsevents a with(nolock) INNER JOIN btevents b with(nolock)
ON a.id = b.id
   AND a.daypart = datepart(dayofyear, @dateday) 
   AND b.daypart = datepart(dayofyear, @dateday)
   AND b.pairedtime BETWEEN dateadd(ss, @timeslack_before , a.timefrom) AND dateadd(ss , @timeslack_after , a.timeto) 
...
from ( SELECT top 100 percent 
	      ...
	      IIF(DATEDIFF(ss , LAG(b.pairedtime) OVER (PARTITION BY b.pairedid ORDER BY b.pairedtime) , b.pairedtime) > @maxtimediff,
	              	        0 , DATEDIFF(ss , LAG(b.pairedtime) OVER (PARTITION BY b.pairedid ORDER BY b.pairedtime) , b.pairedtime)) AS diff, 
	      b.rssi            AS bt_rssi,
 	      b.txpower         AS bt_tx,
	      u2.uuid 	AS bt_uuid
	FROM gpsevents a with(nolock) INNER JOIN btevents b with(nolock)
	...
	WHERE geography::STPointFromText('POINT(' + cast(a.longitude as varchar(20)) + ' ' + cast(a.latitude as varchar(20)) + ')', 4326).STWithin(
      	      geography::STPolyFromText('POLYGON((' + @polygon + '))', 4326)) = 1
	ORDER BY u1.uuid , u2.uuid, a.timefrom
     ) t
GROUP BY t.gps_uuid, t.bt_uuid
)

create function getNumBTpairingsWithinPolygons_statsv2(
	@polygon varchar(8000), 
	@dateday datetime2(0),
	@timeslack_before int,
	@timeslack_after int,
...
create function getTrajectorySpeed3(
	@uuid varchar(40), 
	@timefrom datetime2(0), 
	@timeto datetime2(0)
	)
...
as
return (
select distinct t.id, t.timefrom, t.timeto, t.latitude, t.longitude, accuracy, speed,
       round(f.distancemeters,2) as distancemeters,
       round((iif(f.distancemeters=0,0,isnull(f.distancemeters,0)))/((iif(t.diffsec=0,1,isnull(t.diffsec,1)))),2) as [m/s]
from (select agg_gpsevents.id, timefrom, timeto, latitude, longitude, accuracy, speed, 
	abs(datediff(ss,
		timefrom, -- current row timefrom
		lag(timefrom)  over (order by timefrom))) as diffsec, -- previous row timeto, difference in sec between previous position and current pos
		lag(latitude)  over (order by timefrom)   as prevlat,
		lag(longitude) over (order by timefrom)   as prevlong
	from agg_gpsevents
		join uuid_id on agg_gpsevents.id = uuid_id.id
	where uuid_id.uuid = @uuid
	AND (timefrom between @timefrom and @timeto or timeto between @timefrom and @timeto)
	and daypart between datepart(dayofyear,@timefrom) and datepart(dayofyear,@timeto)  -- search only within the relevant "day-segments" of the table
	) as t
cross apply dbo.fnGetDistanceT(t.latitude,t.longitude,t.prevlat,t.prevlong) as f   -- calculating distance using the function above
);
