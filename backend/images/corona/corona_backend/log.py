...
Adapted from JupyterHub 1.1 (BSD)
"""
...
def log_request(handler):
    ...
    # allow opting out of success logs (e.g. successful health checks)
    if status == 304 or (status < 300 and not getattr(handler, "log_success", True)):
        # health-check success and 304 Found are debug-level
    ...
    try:
        ...
        if user is None:
            ...
            username = mask_phone(user.get("_phonenumber", "unknown"))
        else:
            username = user
    ...
    ns = dict(
        ...
        ip=request.remote_ip.rsplit(".", 1)[0] + ".XXX",
        uri=uri,
